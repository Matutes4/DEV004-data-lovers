...
countries.sort((a, b) => a.pais.localeCompare(b.pais)); // Ordena alfabéticamente por la propiedad "pais"
  ...
  if (pos>-1) {//si la posicion esta en -1 no existe el´pais //
  ...
  alert(`El país ${countryInput} tiene ${countries[pos].Gold} medallas de Oro ${countries[pos].Silver} medallas de plata ${countries[pos].Bronze} medallas de bronce`);
  } else {
    alert("No se encontraron resultados para ese país y medalla"); //si let pos no se encotro el pais arroja esta alerta//
  }
});

function buscar_pais(pais,countries, medalla){//utiliza el bucle
  let i=0;
  for(i=0;i<countries.length;i++){//country buscar  el pais que el ususrio esta ingresando//recorre el elemento de la lista 
    if(countries[i].pais===pais &&countries[i][medalla] > 0){//se comprueba si el nombre del país coincide con el nombre que el usuario ingresó y si el país tiene una cantidad de medallas mayor que cero en la categoría especificada.
      return i;
    }
  }
...
let tablaxdiciplina = function(jugadores, asigna){
  let stringtabla = "";
  stringtabla += "<tr><th>Nombre</th><th>Género</th><th>Deporte</th><th>Edad</th><th>Talla</th><th>Peso</th><th>Oro</th><th>Plata</th><th>Bronce</th></tr>";
