...
export const medal = (data) => {  //se creo una funcion medal para filtrar la data
const paises=[];  //contante paises con un arreglo para ir guardando los paises que no se repiten

for(let p=0; p < data.athletes.length;p++){    //ciclo repetitivo  que recorre el arreglo data.athletes /indicador de la posicion  en el arreglo/numero de posiciones en el arreglo  
  if(data.athletes[p].medal === "Gold" || data.athletes[p].medal === "Silver" || data.athletes[p].medal === "Bronze"){//si el contador inicia en la posicion va recorrer toda la data buscando las medalla de oro plata y bronce 
    ...
    for (let i = 0; i < paises.length; i++) { //for interno ciclo repetitivo  donde el indice inicia 0, y se va incrementando hasta  que sea menor
      if (paises[i].pais === pais) {//el arreglo va ser igual a pais 
        paisIndex = i;
        break;
      }
    }
    ...
    if (paisIndex === -1) {   
      paises.push({ //inserta un valor en la primera posicion del vector//
...
export const atletas = (data) => {
  ...
  for(let p =0; p < data.athletes.length;p++){    //ciclo repetitivo  que recorre el arreglo data.athletes /indicador de la posicion  en el arreglo/numero de posiciones en el arreglo  
    ...
    for (let i = 0; i < jugador.length; i++) { //for interno ciclo repetitivo  donde el indice inicia 0, y se va incrementando hasta  que sea menor

  
      if (jugador[i].name === nombreatleta) {//el arreglo va ser igual a pais 
...
export const atletasUnicos = deportista.filter((atleta, index) => { //propiedades que no se multiplique//
  const firstIndex = deportista.findIndex((obj) => {//traer de su arreglo //
    return obj.name === atleta.name && obj.gender === atleta.gender && obj.sport === atleta.sport && obj.age === atleta.age && obj.height === atleta.height && obj.weight === atleta.weight;
  });
...
export let crearTabla = function(lista){
  let stringtabla = "";
  stringtabla += "<tr><th>Nombre</th><th>Género</th><th>Deporte</th><th>Edad</th><th>Talla</th><th>Peso</th></tr>";
